        ...
        try:
            ...
            if hasattr(result, 'tool_calls') and result.tool_calls:
                ...
                if tool_results:
                    ...
                    valuation_report = final_result.content if hasattr(final_result, 'content') else str(final_result)
                    
                    # 更新messages为最终结果
                    result = final_result
                else:
                    # 如果工具调用失败，返回占位符
                    valuation_report = f"{stock_name}（{stock_code}）估值分析启动，但工具调用失败"
            else:
                # 如果没有工具调用，使用LLM直接回答
                valuation_report = result.content if hasattr(result, 'content') else str(result)
